<?php
$titulo_pagina = 'Novo Carro';
require_once '_header.php';

$conn = conectar();
$id_cliente_pre = (int)($_GET['id_cliente'] ?? 0);
...
$cliente_pre_nome = '';
if ($id_cliente_pre > 0) {
    $stmt = $conn->prepare("SELECT nome FROM clientes WHERE id_cliente = ? AND ativo = 1 LIMIT 1");
    $stmt->bind_param('i', $id_cliente_pre);
    $stmt->execute();
    $r = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if ($r) $cliente_pre_nome = $r['nome'];
    else $id_cliente_pre = 0;
}
$conn->close();
?>
...
<div class="admin-topo-pagina">
    <h1 class="admin-titulo-pagina">
        <a href="carros.php<?php echo $id_cliente_pre > 0 ? '?id_cliente=' . $id_cliente_pre : ''; ?>"
            class="admin-voltar" title="Voltar">
...
<form action="carros_acao.php" method="POST" class="admin-form" id="formCarro">
            ...
            <div class="admin-form-campo admin-form-campo-full">
                ...
                <div class="admin-input-grupo" style="position: relative;">
                    <input type="text" id="busca_cliente" placeholder="Digite o nome do cliente..." autocomplete="off"
                        <?php echo $id_cliente_pre > 0 ? 'disabled' : ''; ?>
                        value="<?php echo limpar($cliente_pre_nome); ?>">
                    <input type="hidden" name="id_cliente" id="id_cliente"
                        value="<?php echo $id_cliente_pre > 0 ? (int)$id_cliente_pre : ''; ?>">
                    <?php if ($id_cliente_pre > 0): ?>
                        <a href="carros_novo.php" class="admin-btn-icone" title="Trocar cliente">
                            <i class="fas fa-times"></i>
                        </a>
                    <?php endif; ?>
                    <div id="listaClientes" class="admin-autocomplete-lista" style="display: none;"></div>
                </div>
                <small class="admin-dica">Digite pelo menos 2 letras do nome.</small>
            </div>
        </div>
    </div>
        ...
        <div class="admin-form-grid">
            <div class="admin-form-campo admin-form-campo-combobox">
                ...
                <div class="admin-combobox" id="comboboxMarca">
                    ...
                    <input type="hidden" name="marca" id="marca_texto" value="">
                ...
                <small class="admin-dica">Digite pra filtrar ou clique na seta pra ver todas.</small>
            </div>

            <div class="admin-form-campo admin-form-campo-combobox">
                <label for="campo_modelo">Modelo *</label>
                <div class="admin-combobox" id="comboboxModelo">
                    <input type="text" id="campo_modelo" placeholder="Escolha a marca primeiro..." autocomplete="off"
                        required disabled>
                    <input type="hidden" name="id_modelo" id="id_modelo" value="">
                    <input type="hidden" name="modelo" id="modelo_texto" value="">
                ...
                <small class="admin-dica">Escolha a marca primeiro.</small>
            </div>

            <div class="admin-form-campo">
                <label for="ano">Ano</label>
                <input type="text" id="ano" name="ano" placeholder="Ex: 2018 ou 2018/2019" maxlength="10">
            </div>

            <div class="admin-form-campo">
                <label for="placa">Placa *</label>
                <input type="text" id="placa" name="placa" placeholder="ABC-1D23 ou ABC1234" required maxlength="10"
                    style="text-transform: uppercase;">
            </div>

            <div class="admin-form-campo">
                <label for="cor">Cor</label>
                <input type="text" id="cor" name="cor" placeholder="Ex: Prata, Preto, Branco..." maxlength="50">
            </div>
    ...
    <div class="admin-form-acoes">
        <a href="carros.php<?php echo $id_cliente_pre > 0 ? '?id_cliente=' . $id_cliente_pre : ''; ?>"
            class="admin-btn admin-btn-secundario">
...
<script>
    (function() {
        ...
        // =========================================================
        // NAVEGAÇÃO POR TECLADO (GLOBAL — funciona mesmo sem foco no input)
        // =========================================================
        ...
        // =========================================================
        // MARCA — TECLADO (captura global, funciona sem foco no input)
        // =========================================================
        document.addEventListener('keydown', (e) => {
            const abertoMarca = listaMarca.style.display === 'block';
            const abertoModelo = listaModelo.style.display === 'block';

            // Se a lista da marca tá aberta e o foco NÃO tá no input da marca
            ...
            // Se a lista do modelo tá aberta e o foco NÃO tá no input do modelo
